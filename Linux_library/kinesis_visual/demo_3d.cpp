...
char kinesisRecord[256];
FILE *kinesisStream;
int streamOpen = 0;
pthread_t kinesisThread;
...
float badgerloop(void) {
    int currValue = 0; //ax = 0, ay = 1, az = 2
    while (fgets(kinesisRecord, sizeof(kinesisRecord), kinesisStream) != NULL) {
    	if (streamOpen == 0 && (strcmp(kinesisRecord, "__BADGERLOOP__") == 0 || strcmp(kinesisRecord, "end") == 0))
        {
        	streamOpen = 1;
        }
        if (streamOpen != 1)
        ...
        if (currValue == 0)
        {
        	ypr[2] = strtol(kinesisRecord, NULL, 10);
        	currValue++;
        	return (float)strtol(kinesisRecord, NULL, 10);
        	//continue;
        } else if (currValue == 1) {
        	ypr[1] = strtol(kinesisRecord, NULL, 10);
        	currValue++;
        	continue;
        } else if (currValue == 2) {
        	ypr[0] = strtol(kinesisRecord, NULL, 10);
        	currValue++;
        	continue;
        }
    }
...
void setup() {
    ...
    if (devStatus == 0) {
        ...
        kinesisStream = popen("python worker.py badgerloop_testing --echo", "r");
        //pthread_create( &kinesisThread, NULL, badgerloop, (void*) 0);
