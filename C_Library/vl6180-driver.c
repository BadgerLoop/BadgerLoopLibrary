...
void setMode(VL6180_MODE mode)
{
    ...
    switch (mode) {
        ...
        case (ALS_CONTINUOUS):
            clearModes();
            I2Cdev_writeByte(0x29, SYSALS_START, 0x03);
            //writeOne(I2C, SYSALS_START, 0x03);
            break;
        case (RANGE_CONTINUOUS):
            clearModes();
            I2Cdev_writeByte(0x29, SYSRANGE_START, 0x03);
        ...
        case (RANGE_SINGLE_SHOT_AND_ALS_CONTINUOUS):
            ...
            break;
        case (INTERLEAVED):
            I2Cdev_writeByte(0x29, INTERLEAVEd_MODE_ENABLE, 0x01);
...
float singleShotReadALS()
{
    ...
    I2Cdev_writeByte(0x29, SYSALS_START, 0x01);
    //writeOne(I2C, SYSALS_START, 0x01);
    while (!newALS_Data());
    I2Cdev_readBytes(0x29, RESULT_ALS_VAL, 4, newData);
...
uint8_t singleShotReadRange()
{
    ...
    I2Cdev_writeByte(0x29, SYSRANGE_START, 0x01);
    while (!newRangeData);
    I2Cdev_readByte(0x29, RESULT_RANGE_VAL, &newData);
...
void setSysALS_IntegrationPeriod(int ms)
{
    ...
    I2Cdev_writeByte(0x29, SYSALS_INTEGRATION_PERIOD, encodedMs);
...
{
    uint8_t readStatus;
    I2Cdev_readByte(0x29, RESULT_INTERRUPT_STATUS, &readStatus);
